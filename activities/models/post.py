...
class Post(StatorModel):
    ...
    def handle_quote_request_ap(cls, data):
        ...
        if post.visibility in (cls.Visibilities.public, cls.Visibilities.unlisted):
            # Auto-accept: send Accept with QuoteAuthorization
            ...
            # Reject quotes for non-public posts
            reject_data = {
                "type": "Reject",
                "id": (f"{post.author.actor_uri}#reject-{Snowflake.generate_post()}"),
                "actor": post.author.actor_uri,
                "object": data.get("id", actor_uri),
            }
            try:
